        ...
        for details in response["data"]:
            for VO_details in details["Vo"][6]["VoAcknowledgments"]:
                ...
                if VO_details["VoAcknowledgment"][3]["publicationUrl"]:
                    publicationsURL = VO_details["VoAcknowledgment"][3][
                        "publicationUrl"
                    ]
                else:
                    publicationsURL = "N/A"
...
def get_VO_stats(env, vo):
    ...
    if response:
        for details in response["data"]:
            # print("%d.) Fetching metadata for the VO [%s] in progress.." %(index, details['name']))
            if vo in details["name"]:
                statement, publicationsURL, index = get_VO_metadata(
                    index, env, details["name"]
                )
                ...
                vo_stats.append(
                    {
                        ...
                        "users": get_VO_users(
                            env, details["name"]
                        ),  # Current users in the VO
                        "active_members": details["members"],  # Active users in the VO
                        # "total_members" : details['totals'], # Total users in the VO
...
def get_VOs_stats(env):
    ...
    if response:
        ...
        for details in response["data"]:
            if env["LOG"] == "DEBUG":
                print(
                    "\n%d.) Fetching metadata for the VO [%s] in progress.."
                    % (index, details["name"])
                )
            else:
                print(
