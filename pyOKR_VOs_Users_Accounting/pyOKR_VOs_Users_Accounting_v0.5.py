...
__date__      = "$Date: 07/06/2024 18:23:17"
...
def update_GWorkSheet_VOs(env, worksheet, VOs_list, accounting_period):
        ...
        while index < len(VOs_list):
            try:
                if worksheet_dict['VO'] == VOs_list[index]['name']:
                   ...
                   worksheet.update_cell(VO_cell.row, Period_cell.col, VOs_list[index]['users'])

                   # Update the Google Worksheet cell (with the 'Registered' users)
                   worksheet.update_cell(VO_cell.row, Registered_Users_cell.col, VOs_list[index]['active_members'])
                   
                   # Update the Google Worksheet cell (with the 'Total' users)
                   worksheet.update_cell(VO_cell.row, Total_Users_cell.col, VOs_list[index]['total_members'])
          
                   if env['LOG'] == "DEBUG":
                      print(colourise("green", "[LOG]"), \
                      "Updated statistics (%s, %s, %s) for the VO [%s]" \
                      %(VOs_list[index]['users'], VOs_list[index]['active_members'], VOs_list[index]['total_members'], \
                      VOs_list[index]['name']))
    ...
    print(colourise("cyan", "[INFO]"), " Existing VOs statistics updated in the Google worksheet!")
    ...
    Registered_Users_index = get_GWorkSheet_HeaderPosition(env, worksheet, "Registered Users")
    ...
    while index < len(VOs_list):
      try:
          ...
          print("Insert the VO [%s] at: (%d, %d)" %(VOs_list[index]['name'], row_index, col_index))
          ...
          # Update the Google Worksheet cell (with the 'Registered' users)
          worksheet.update_cell(row_index, Registered_Users_cell.col, VOs_list[index]['active_members'])
          
          # Update the Google Worksheet cell (with the 'Total' users)
          worksheet.update_cell(row_index, Total_Users_cell.col, VOs_list[index]['total_members'])
...
def main():
    ...
    print(colourise("cyan", "\n[INFO]"), \
            "\tReporting Period: '%s'" %accounting_period)
