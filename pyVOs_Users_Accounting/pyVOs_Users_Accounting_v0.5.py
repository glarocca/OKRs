...
def update_GWorkSheet_VOs(env, worksheet, VOs_list, accounting_period):
    ...
    Active_Users_cell = worksheet.find('Active Users')
        ...
        while index < len(VOs_list):
            try:
                if worksheet_dict['VO'] == VOs_list[index]['name']:
                   ...
                   # Update the Google Worksheet cell (with the 'Active' users)
                   worksheet.update_cell(VO_cell.row, Active_Users_cell.col, VOs_list[index]['active_members'])
    ...
    Active_Users_index = get_GWorkSheet_HeaderPosition(env, worksheet, "Active Users")
    ...
    while index < len(VOs_list):
      try:
          ...
          # Update the Google Worksheet cell (with the 'Active' users)
          worksheet.update_cell(row_index, Active_Users_cell.col, VOs_list[index]['active_members'])
    ...
    print(colourise("cyan", "[INFO]"), " [%d] *new* VO statistics added in the Google worksheet!" %len(VOs_list))
