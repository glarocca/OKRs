...
def init_GWorkSheet(env):
    ...
    try:
        ...
        if "egi" in env['ACCOUNTING_SCOPE']:
           worksheet = sheet.worksheet(env['GOOGLE_SLAs_HTC_WORKSHEET'])
        else:   
           worksheet = sheet.worksheet(env['GOOGLE_SLAs_CLOUD_WORKSHEET'])
        ...
        print(colourise("red", "[ABORT]"), \
              "The env['GOOGLE_SLAs_CLOUD|HTC_REPORT_WORKSHEET'] setting were *NOT FOUND*")
        print("\tPlease check environmental settings and try again!")
