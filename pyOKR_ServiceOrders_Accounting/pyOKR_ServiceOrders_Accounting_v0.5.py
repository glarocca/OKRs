...
__date__      = "$Date: 07/06/2024 06:55:17"
...
def update_GWorkSheet_SOs(env, worksheet, EGI_SOs, reporting_period):
    ...
    while not update:
        for worksheet_dict in worksheet_dicts:
            try:
               if worksheet_dict['Services'] == 'EGI Cloud Compute':
                  ...
                  worksheet.update_cell(Cloud_Compute_cell.row, 
                    Period_cell.col, 
                  ...
                  print("Updated the #SOs [%d] for the service: EGI Cloud Compute" \
                        %len(EGI_SOs['EGI Cloud Compute']))
               ...
               if worksheet_dict['Services'] == 'EGI Cloud Container Compute':
                  ...
                  Cloud_Container_Compute_cell = worksheet.find('EGI Cloud Container Compute')
                  worksheet.update_cell(Cloud_Container_Compute_cell.row, 
                    Period_cell.col, 
                    len(EGI_SOs['EGI Cloud Container Compute']))

                  # Converting row and column cell address to A1 notation
                  worksheet.insert_note(
                       gspread.utils.rowcol_to_a1(Cloud_Container_Compute_cell.row, Period_cell.col),
                       service_orders)

                  print("Updated the #SOs [%d] for the service: EGI Cloud Container Compute" \
                        %len(EGI_SOs['EGI Cloud Container Compute']))
               ...
               if worksheet_dict['Services'] == 'EGI High-Throughput Compute':
                  ...
                  High_Throughput_Compute_cell = worksheet.find('EGI High-Throughput Compute')
                  worksheet.update_cell(High_Throughput_Compute_cell.row,
                    Period_cell.col,
                    len(EGI_SOs['EGI High-Throughput Compute']))

                  # Converting row and column cell address to A1 notation
                  worksheet.insert_note(
                       gspread.utils.rowcol_to_a1(High_Throughput_Compute_cell.row, Period_cell.col),
                       service_orders)

                  print("Updated the #SOs [%d] for the service: EGI High-Throughput Compute" \
               ...
               if worksheet_dict['Services'] == 'EGI Software Distribution':
                  ...
                  worksheet.update_cell(Software_Distribution_cell.row,
                    Period_cell.col,
                    len(EGI_SOs['EGI Software Distribution']))

                  # Converting row and column cell address to A1 notation
                  worksheet.insert_note(
                       gspread.utils.rowcol_to_a1(Software_Distribution_cell.row, Period_cell.col),
                       service_orders)

                  print("Updated the #SOs [%d] for the service: EGI Software Distribution" \
                       %len(EGI_SOs['EGI Software Distribution']))
               ...
               if worksheet_dict['Services'] == 'EGI Workload Manager':
                  ...
                  worksheet.update_cell(Workload_Manager_cell.row,
                    Period_cell.col,
                    len(EGI_SOs['EGI Workload Manager']))

                  # Converting row and column cell address to A1 notation
                  worksheet.insert_note(
                       gspread.utils.rowcol_to_a1(Workload_Manager_cell.row, Period_cell.col),
                       service_orders)

                  print("Updated the #SOs [%d] for the service: EGI Workload Manager" \
                       %len(EGI_SOs['EGI Workload Manager']))
               ...
               if worksheet_dict['Services'] == 'EGI Infrastructure Manager':
                  ...
                  Infrastructure_Manager_cell = worksheet.find('EGI Infrastructure Manager')
                  worksheet.update_cell(Infrastructure_Manager_cell.row,
                    Period_cell.col,
                    len(EGI_SOs['EGI Infrastructure Manager']))

                  # Converting row and column cell address to A1 notation
                  worksheet.insert_note(
                       gspread.utils.rowcol_to_a1(Infrastructure_Manager_cell.row, Period_cell.col),
                       service_orders)

                  print("Updated the #SOs [%d] for the service: EGI Infrastructure Manager" \
                       %len(EGI_SOs['EGI Infrastructure Manager']))  
               ...
               if worksheet_dict['Services'] == 'EGI Online Storage':
                  ...
                  worksheet.update_cell(Online_Storage_cell.row,
                    Period_cell.col,
                    len(EGI_SOs['EGI Online Storage']))

                  # Converting row and column cell address to A1 notation
                  worksheet.insert_note(
                       gspread.utils.rowcol_to_a1(Online_Storage_cell.row, Period_cell.col),
                       service_orders)

                  print("Updated the #SOs [%d] for the service: EGI Online Storage" \
                       %len(EGI_SOs['EGI Online Storage']))
               ...
               if worksheet_dict['Services'] == 'EGI Data Transfer':
                  ...
                  worksheet.update_cell(Data_Transfer_cell.row,
                    Period_cell.col,
                  ...
                  print("Updated the #SOs [%d] for the service: EGI Data Transfer" \
                       %len(EGI_SOs['EGI Data Transfer']))
               ...
               if worksheet_dict['Services'] == 'EGI DataHub':
                  ...
                  worksheet.update_cell(DataHub_cell.row,
                    Period_cell.col,
                  ...
                  print("Updated the #SOs [%d] for the service: EGI DataHub" \
                       %len(EGI_SOs['EGI DataHub']))
               ...
               if worksheet_dict['Services'] == 'EGI Check-In':
                  ...
                  worksheet.update_cell(Check_In_cell.row,
                    Period_cell.col,
                  ...
                  print("Updated the #SOs [%d] for the service: EGI Check-In" \
                       %len(EGI_SOs['EGI Check-In']))
               ...
               if worksheet_dict['Services'] == 'EGI Notebooks':
                  ...
                  worksheet.update_cell(Notebooks_cell.row,
                    Period_cell.col,
                  ...
                  print("Updated the #SOs [%d] for the service: EGI Notebooks" \
                       %len(EGI_SOs['EGI Notebooks']))
               ...
               if worksheet_dict['Services'] == 'EGI Replay':
                  ...
                  worksheet.update_cell(Replay_cell.row,
                    Period_cell.col,
                  ...
                  print("Updated the #SOs [%d] for the service: EGI Replay" \
                       %len(EGI_SOs['EGI Replay']))
               ...
               if worksheet_dict['Services'] == 'EGI FitSM Training':
                  ...
                  worksheet.update_cell(FitSM_Training_cell.row,
                    Period_cell.col,
                    len(EGI_SOs['EGI FitSM Training']))

                  # Converting row and column cell address to A1 notation
                  worksheet.insert_note(
                       gspread.utils.rowcol_to_a1(FitSM_Training_cell.row, Period_cell.col),
                       service_orders)

                  print("Updated the #SOs [%d] for the service: EGI FitSM Training" \
                       %len(EGI_SOs['EGI FitSM Training']))
               ...
               if worksheet_dict['Services'] == 'EGI ISO 27001 Training':
                  ...
                  worksheet.update_cell(ISO_27001_Training_cell.row,
                    Period_cell.col,
                    len(EGI_SOs['EGI ISO 27001 Training']))

                  # Converting row and column cell address to A1 notation
                  worksheet.insert_note(
                       gspread.utils.rowcol_to_a1(ISO_27001_Training_cell.row, Period_cell.col),
                       service_orders)

                  print("Updated the #SOs [%d] for the service: EGI ISO 27001 Training" \
                       %len(EGI_SOs['EGI ISO 27001 Training']))
               ...
               if worksheet_dict['Services'] == 'EGI Training Infrastructure':
                  ...
                  Training_Infrastructure_cell = worksheet.find('EGI Training Infrastructure')
                  worksheet.update_cell(Training_Infrastructure_cell.row,
                    Period_cell.col,
                    len(EGI_SOs['EGI Training Infrastructure']))

                  # Converting row and column cell address to A1 notation
                  worksheet.insert_note(
                       gspread.utils.rowcol_to_a1(Training_Infrastructure_cell.row, Period_cell.col),
                       service_orders)

                  print("Updated the #SOs [%d] for the service: EGI Training Infrastructure" \
                       %len(EGI_SOs['EGI Training Infrastructure']))
               ...
               if worksheet_dict['Services'] == 'EGI Dynamic DNS':
                  ...
                  worksheet.update_cell(Dynamic_DNS_cell.row,
                    Period_cell.col,
                  ...
                  print("Updated the #SOs [%d] for the service: EGI Dynamic DNS" \
        ...
        print(colourise("cyan", "[INFO]"), " Service Orders updated in the Google worksheet!")


    # 2.) Insert SOs for the EGI services *not* existing in the worksheet
    if len(EGI_SOs) > 0:
       print(colourise("cyan", "\n[INFO]"), \
    ...
    for service, SOs in EGI_SOs.items():
        try:
            ...
            print("Insert the EGI Service [%s] at: (%d, %d)" %(service, row_index, col_index))
...
def main():
    ...
    reporting_period = env['DATE_FROM'][0:4] + "." \
            + env['DATE_FROM'][-2:] + "-" \
            + env['DATE_TO'][-2:]
    ...
    for service_order in service_orders:
        ...
        if env['SERVICE_ORDERS_ISSUETYPE'] in service_order['fields']['issuetype']['name']:
           ...
           if env['LOG'] == "DEBUG":
               ...
               "Service Order (SO) received via the EOSC Marketplace [%d]" %len(service_orders_list))
    ...
    if env['LOG'] == "DEBUG":
       print(colourise("cyan", "\n[INFO]"), \
       " EOSC service orders received for the EGI services in the reporting period %s" %reporting_period)
